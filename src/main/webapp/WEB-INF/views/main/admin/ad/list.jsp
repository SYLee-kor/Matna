...
<body>
	<center>
		...
		<table id="Table" align="center" border="1">
			<tr>
				<th width="50"><input type="checkbox" name="chkAll"
			...
			<c:forEach var="ad" items="${ad}" varStatus="stat">
				<tr height="1" align="center">
					<td><input type="checkbox" name="check" value="${ad.no}" /> <input
						type='hidden' name="delNo" value="${ad.no}"></td>
					<td>${ad.no}</td>
					<td><a
						href="/matna/admin/AD/read?no=${ad.no }&page=${pageMaker.cri.page}&searchType=${pageMaker.cri.searchType}&keyword=${pageMaker.cri.keyword}">${ad.title }</a>
					</td>
				</tr>
			</c:forEach>
		</table>
